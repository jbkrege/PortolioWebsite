...
function ProjectTile(_ref) {
    ...
    return React.createElement(
        ...
        React.createElement(
            'a',
            { href: proj.href },
            React.createElement('img', { className: 'proj-pic', src: proj.projPic }),
            React.createElement('img', { className: 'proj-gif', src: proj.projGif }),
            React.createElement(
                'div',
                { className: 'proj-underlay' },
                React.createElement(
                    'div',
                    { className: 'proj-underlay-text' },
                    proj.projText
                )
            )
        )
...
function ProjectGrid() {
    ...
    for (var ii = 0; ii < projectInfo.length; ii = ii + 2) {
        ...
        columnsInRow.push(React.createElement(ProjectTile, { proj: currProj, key: currProj.name }));

        // add second project in row if available
        if (ii + 1 < projectInfo.length) {
            var nextProj = projectInfo[ii + 1];
            columnsInRow.push(React.createElement(ProjectTile, { proj: nextProj, key: nextProj.name }));
        }

        rows.push(React.createElement(
            'div',
            { className: 'row', key: "row" + ii / 2 },
            columnsInRow
        ));
    }
...
var app = document.getElementById('proj-grid');
ReactDOM.render(React.createElement(ProjectGrid, null), app);